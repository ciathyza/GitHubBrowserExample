...
class RepositoryDetailViewController:UIViewController, StoreSubscriber
{
	...
	override func viewWillAppear(_ animated:Bool)
	{
		...
		store.subscribe(self)
		{
			state in
				...
				{
					currentState in
						let currentRepository:Repository? = currentState.navigationState.getRouteSpecificState( currentState.navigationState.route )
						let isCurrentRepositoryBookmarked = currentRepository.map { BookmarkService.isRepositoryBookmarked(state: currentState, currentRepository: $0) } ?? false
						return ( currentRepository, isCurrentRepositoryBookmarked )
				}
		}
	}
	...
	{
		store.dispatch( CreateBookmark( route: [mainViewRoute, repositoryDetailRoute], routeSpecificData: self.repository ) )
	}
}
